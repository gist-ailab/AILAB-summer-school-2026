    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            if reached:
                ...
                # ======================== 파지 예측 ========================
                # Depth → point cloud 생성 (RGB 컬러도 함께)
                rgb_for_pc = np.array(img_np)[..., :3]
                pc, pc_colors = depth2pc(depth_np, K, rgb=rgb_for_pc)

                # 타겟 마스크를 point cloud 점 단위 마스크로 변환
                # (depth2pc는 depth>0 픽셀을 행 우선으로 담으므로 동일 마스킹으로 1:1 대응)
                obj_mask_img = pred_masks[best_idx, 0] > 0.5
                valid_depth = depth_np > 0
                pc_obj_mask = obj_mask_img[valid_depth]
                ...
                # (B) Contact-GraspNet 추론 → 파지 회전행렬/위치/폭
                ...
                # approach축 180° 대칭 해소: 2지 그리퍼는 approach(z)축 180° 회전에 동일한 파지.
                # 현재 손 자세에 더 가까운 쪽을 선택해 손목 꺾임/self-collision 회피.
                ...
                # (C) 회전행렬 → 쿼터니언 (scipy는 xyzw → Isaac은 wxyz)
                ...
                # (C) approach 방향(z축)으로 grasp 깊이 offset + pregrasp(뒤로 0.1m)
