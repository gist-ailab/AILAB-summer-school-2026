# day2_2.3_get_observations_answer.py
#
# 실행 방법:
#   isaaclab -p day2_2.3_get_observations_answer.py
#
# 실행 후 확인:
#   observations/ 폴더에 rgb, depth, segmentation mask, bbox 결과가 저장됨
...
OBJECT_LABELS = {
    f"{BASE_SCENE_PRIM_PATH}/Object1": "object1",
    f"{BASE_SCENE_PRIM_PATH}/Object2": "object2",
    f"{BASE_SCENE_PRIM_PATH}/Object3": "object3",
}

# bbox 저장 시 object1, object2, object3만 남기기 위한 target label 목록
...
def create_existing_camera_sensor():
    # -------------------------------------------------------------------------
    # 5. 기존 Camera를 Isaac Lab Sensor로 연결
    # -------------------------------------------------------------------------
    ...
    # distance_to_camera 내부 처리에서 camera.cfg.spawn.clipping_range를 참조하므로
    # 기존 Camera prim을 새로 만들지는 않되, 설정값만 채워줌.
    camera.cfg.spawn = sim_utils.PinholeCameraCfg(
        focal_length=12.0,
...
def save_one_observation_frame(output_dir: Path, camera: Camera):
    ...
    # 1. RGB 저장
    rgb = output["rgb"][0, ..., :3].detach().cpu().numpy().astype(np.uint8)
    Image.fromarray(rgb).save(output_dir / "rgb.png")

    # 2. Depth 저장
    depth = output["distance_to_camera"][0].detach().cpu().numpy().squeeze()
    ...
    # 3. Semantic segmentation mask 저장
    semantic = output["semantic_segmentation"][0].detach().cpu().numpy().squeeze().astype(np.int32)
    ...
    # 4. Semantic mask에서 target object의 visible 2D bbox만 계산
    ...
    for semantic_id in np.unique(semantic):
        ...
        # table, material, mesh, collisionmesh, UNLABELLED 등은 저장하지 않음
        if object_label is None:
            continue

        ys, xs = np.where(semantic == semantic_id)
