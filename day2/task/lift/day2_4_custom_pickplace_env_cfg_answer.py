...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    bin = RigidObjectCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path=str(DATA_DIR / "assets" / "basket" / "basket.usd"),
            scale=(0.8, 0.25, 0.8),
    ...
    def __post_init__(self):
        ...
        # YCB object들 전체 읽어오기
        ycb_obj_usd_paths = glob.glob(str(DATA_DIR / "assets" / "YCB_usd_for_panda_selected" / "*" / "textured.usd"))

        # YCB object 중 3가지 물체 random하게 설정
        selected_ycb_obj_usd_paths = random.sample(ycb_obj_usd_paths, len(objects_position))
        ...
        for i in range(len(selected_ycb_obj_usd_paths)):
            ...
            # object config 세팅 (physics_material에서 friction을 설정해줘야 물체를 잘 잡을 수 있음)
