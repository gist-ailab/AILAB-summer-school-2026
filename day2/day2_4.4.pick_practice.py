    ...
    def compute(self, ee_pose, grasp_pose, pregrasp_pose):
        ...
        for i in range(self.num_envs):
            ...
            ############## TODO: PREGRASP 상태 ##############
                ...
                # 목표: pregrasp_pose[i]로 이동, 그리퍼 열기(OPEN)
                # 전환: 도달(임계값) + 대기(PickSmWaitTime.PREGRASP) → PickSmState.GRASP

            ################################################

            ############## TODO: GRASP 상태 ##############
            elif state == PickSmState.GRASP:
                # 목표: 예측된 grasp_pose[i]로 접근, 그리퍼는 아직 열기(OPEN)
                # 전환: 도달 + 대기(PickSmWaitTime.GRASP) → PickSmState.CLOSE

            #############################################

            ############## TODO: CLOSE 상태 ##############
            elif state == PickSmState.CLOSE:
                # 목표: 현재 자세(ee_pose[i]) 유지, 그리퍼 닫기(CLOSE)로 물체 집기
                # 전환: 대기(PickSmWaitTime.CLOSE) → PickSmState.LIFT
                #       이때 들어올릴 목표 self.lift_pose 설정: grasp_pose[i] 복사 후 z += 0.4

            #############################################

            ############## TODO: LIFT 상태 ##############
            elif state == PickSmState.LIFT:
                # 목표: self.lift_pose로 들어올린 채 유지, 그리퍼 닫기(CLOSE). (Step4는 여기서 끝)
...
def main():
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            # -- PREDICT state에서 한 번만 SAM3 + GraspNet 수행 (Step 1~3 내용, 제공됨) --
            if pick_sm.sm_state[env_num] == PickSmState.PREDICT and not predicted:
                ...
                with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    inference_state = sam3_processor.set_image(Image.fromarray(img_rgb))
