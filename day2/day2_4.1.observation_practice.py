...
def main():
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            ############## TODO: 관측 자세 유지 액션 구성 ##############
            # observe_pose(7)와 gripper_open(1)을 이어붙여 (num_envs, 8) 액션을 만든다.
            # 힌트: torch.cat([...], dim=-1)
            actions =
            #######################################################
            ...
            if reached and not saved:
                ...
                ############## TODO: 손목 카메라에서 RGB / Depth 취득 ##############
                # robot_camera.data.output 딕셔너리에서 아래 두 값을 꺼내 numpy 배열로 만든다.
                #  - "rgb"                     : (H, W, 3) 컬러 이미지
                #  - "distance_to_image_plane" : (H, W) depth 이미지
                # 힌트: robot_camera.data.output["<key>"][0] → .detach().cpu().numpy()
                rgb =
                depth =
                ###############################################################
