...
class PickAndPlaceSm:
    ...
    def compute(self, ee_pose: torch.Tensor, grasp_pose: torch.Tensor, pregrasp_pose: torch.Tensor):
        ...
        for i in range(self.num_envs):
            ...
            # ==========================================================================
            # [놓기(Place)] 아래 5개 state를 채우기.
            #  각 상태: (1) self.des_ee_pose[i] 목표 자세 (2) self.des_gripper_state[i]
            #           (3) 도달(임계값) + 대기시간 충족 시 다음 상태로 전환 + 대기시간 0으로 초기화
            #  참고 자세: self.bin_pose(바구니 위), self.bin_lower_pose(바구니 안 낮은 자세),
            #            self.capture_pose(관측 자세)
            # ==========================================================================
            ########## TODO: MOVE_TO_BIN 상태 ##########
            elif state == PickAndPlaceSmState.MOVE_TO_BIN:
                # 목표: 바구니 위 자세(self.bin_pose[i])로 이동, 그리퍼 닫기(집은 상태 유지)
                # 전환: 도달 + 대기(MOVE_TO_BIN) → LOWER

            ###########################################

            ########## TODO: LOWER 상태 ##########
            elif state == PickAndPlaceSmState.LOWER:
                # 목표: 바구니 안 낮은 자세(self.bin_lower_pose[i])로 내리기, 그리퍼 닫기
                # 전환: 도달 + 대기(LOWER) → RELEASE

            #####################################

            ########## TODO: RELEASE 상태 #########
            elif state == PickAndPlaceSmState.RELEASE:
                # 목표: 낮은 자세 유지, 그리퍼 열기(OPEN)로 물체 놓기
                # 전환: 도달 + 대기(RELEASE) → BACK

            ######################################

            ########### TODO: BACK 상태 ###########
            elif state == PickAndPlaceSmState.BACK:
                # 목표: 다시 바구니 위 자세(self.bin_pose[i])로 올라오기, 그리퍼 열기
                # 전환: 도달 + 대기(BACK) → BACK_TO_READY

            ######################################

            ########## TODO: BACK_TO_READY 상태 ##########
            elif state == PickAndPlaceSmState.BACK_TO_READY:
                # 목표: 관측 자세(self.capture_pose[i])로 복귀, 그리퍼 열기
                # 전환: 도달 or 대기(BACK_TO_READY) → REST (남은 물체를 잡으러 처음으로)
