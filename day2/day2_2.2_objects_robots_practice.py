# day2_2.2_objects_robots_practice.py
#
# 실행 방법:
#   isaaclab -p day2_2.2_objects_robots_practice.py
#
# 목표:
#   이전 시간에 만든 tabletop scene을 불러오고,
#   table 위에 object들과 custom robot을 배치함
...
@configclass
# Problem1
class ObjectsRobotsSceneCfg(___________________):
    """이전 시간에 만든 tabletop scene 위에 object와 robot을 추가함."""

    # -------------------------------------------------------------------------
    # 2. 이전 시간에 만든 Tabletop Scene 불러오기
    # -------------------------------------------------------------------------
    # GUI:
    #   2_2.1 시간에 Ground, Table, Light, Camera를 만들고 scene으로 저장
    #
    # Code:
    #   저장된 tabletop USD를 하나의 asset으로 scene에 등록

    tabletop = ____________(
        __________="{ENV_REGEX_NS}/Tabletop",
        spawn=sim_utils.__________(
            ________=(DAY2_DIR / "scenes/2_2.1_tabletop.usda").as_posix(),
        ),
        __________=AssetBaseCfg.InitialStateCfg(
            pos=(0.0, 0.0, 0.0),
            rot=(1.0, 0.0, 0.0, 0.0),
        ),
    )
...
# Problem 2
    ...
    # -------------------------------------------------------------------------
    # GUI:
    #   Content 창에서 object USD를 drag & drop
    #   Transform에서 위치와 크기 조정
    #
    # Code:
    #   Object1을 물리 상태를 가진 rigid object로 등록

    object1 = ______________(
        __________="{ENV_REGEX_NS}/Object1",
        spawn=sim_utils.__________(
            usd_path=(DAY2_DIR / "assets/objects/006_mustard_bottle/final.usd").as_posix(),
            ______=(1.5, 1.5, 1.5),
            ___________=sim_utils.RigidBodyPropertiesCfg(),
            ______________=sim_utils.CollisionPropertiesCfg(),
        ),
...
# Problem 3
    ...
    #
    # Code:
    #   joint와 link가 있는 robot을 articulation으로 불러와 배치

    robot = ______________(
...
def main():
    ...
    print("[INFO] GUI에서 /World/envs/env_0/Object1, Object2, Object3, Robot을 확인하세요.")
