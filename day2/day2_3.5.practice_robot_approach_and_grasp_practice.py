"""
Isaac Lab 환경에서 Franka 로봇을 사용한 GRASP OBJECT 예제

1. Isaac Lab 시뮬레이션 환경 초기화
2. Franka 로봇과 큐브가 포함된 리프팅 환경 생성
3. State Machine을 통한 로봇의 GRASP OBJECT 제어
"""
...
class PickAndLiftSm:
    """
    로봇의 태스크 공간에서 오브젝트를 잡는 간단한 State Machine
...
def main():
    ...
    # 6. 시뮬레이션 메인 루프
    # ============================================================================
    # LIFT 없이 APPROACH -> GRASP까지만 수행한다. (들어올림 목표 계산 불필요)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # LIFT를 하지 않으므로 별도의 들어올림 목표(desired_position)는 필요 없다.
            # compute() 시그니처 유지를 위해 오브젝트 위치를 그대로 전달한다.
            ...
            actions = pick_sm.compute(
                ...
                des_object_pos=None,                                           # GRASP 까지만 수행하므로 들어올림 목표는 필요 없음
                robot_data=robot_data                                          # 로봇 데이터
                )
