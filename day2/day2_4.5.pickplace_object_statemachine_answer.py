"""
놓기(Place) / 전체 통합(SAM3 텍스트 검출 + Contact-GraspNet 파지 예측 + State Machine 제어)


Step 1~4에서 만든 관측·검출·파지예측·집기를 하나로 잇고,
물체를 바구니에 놓고 남은 물체까지 반복하는 최종 pick-and-place.

* statemachine 코드 부분이 많아 day2/utils/day2_4_statemachine_answer.py 파일로 분리
"""
...
from day2.utils.day2_4_statemachine_answer import PickAndPlaceSm, PickAndPlaceSmState
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # 모든 물체가 들어갔으면 종료하지 않고 리셋 후 계속 진행
            dones = (len(object_names) > 0) and (len(objects_in_bin) == len(object_names))

            if dones:
                print("[INFO] All objects placed in the bin. Resetting...")
                env.reset()                      # 씬(로봇/물체/바구니)을 초기 상태로 복귀
                pick_and_place_sm.reset_idx()    # 상태 머신을 REST로 리셋
                objects_in_bin.clear()           # 물체가 테이블로 돌아왔으므로 바구니 추적 상태 초기화
