"""
Isaac Lab 환경에서 Franka 로봇을 사용한 파지 예제

1. Isaac Lab 시뮬레이션 환경 초기화
2. Franka 로봇과 큐브가 포함된 리프팅 환경 생성
3. State Machine을 통한 로봇의 파지 자세 제어
"""
...
from task.lift.config.day2_3_ik_abs_env_cfg_answer import FrankaCubeLiftEnvCfg
...
def main():
    ...
    # 6. 시뮬레이션 메인 루프
    # ============================================================================
    # 들어올림 목표: 물체 초기 위치를 "로봇 base 기준"으로 잡고 z에 offset (command_manager 대체)
    # LIFT 상태는 목표를 base 프레임 그대로 IK에 넣으므로, 월드(env-local)가 아니라 로봇 base
    # 기준으로 맞춰야 로봇 base 높이(z=0.5)가 이중으로 더해져 너무 높이 올라가는 것을 막는다.
    LIFT_OFFSET = 0.3   # 로봇 base 기준, 물체를 이 높이만큼 들어올림(상대 리프트)
    while simulation_app.is_running():
        # 모든 연산을 추론 모드에서 실행 (메모리 효율성)
        with torch.inference_mode():
            # 환경 스텝 실행
            if pick_sm.sm_state[0] == PickSmState.REST:
                for _ in range(20):  # 초기화 루프: 물체 초기 위치를 안정적으로 잡기 위해 20 스텝 동안 환경을 스텝
                    obs, _ = env.step(actions)
                object_init_pos = (env.scene["object"].data.root_pos_w - env.scene.env_origins).clone()
            ...
            # command_manager 대신: 물체 초기 위치(xy) 기준 z 오프셋을 들어올림 목표로 사용
            desired_position = object_init_pos.clone()
            desired_position[..., 2] += LIFT_OFFSET
            ...
            # 종료 조건 확인
            if torch.all(torch.abs(object_position[..., :] - desired_position[..., :]) < 0.01):  # 물체가 목표 위치에 도달하면
                print("Environment done. Resetting...")
                env.reset()
                pick_sm.reset_idx()  # 상태 머신 리셋
