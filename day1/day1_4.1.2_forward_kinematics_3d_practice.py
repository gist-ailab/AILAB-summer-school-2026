...
def forward_kinematics(theta1, theta2, theta3, L0=0.5, L1=1.0, L2=1.0):
    ...
    """
    # ──────────────────────────────────────────────────────────
    # [실습] 3D 팔의 정운동학을 구현하세요.
    #   theta1=base yaw(수직축 회전), theta2=shoulder, theta3=elbow pitch.
    #   아이디어: pitch(theta2,theta3)로 "세로 평면"에서 수평거리 r 과 높이 z 를
    #             먼저 구하고, yaw(theta1)로 XY 평면에 배치함.
    #   p0 (Base)     = (0, 0, 0)
    #   p1 (Shoulder) = (0, 0, L0)              # 수직 기둥 끝
    #   p2 (Elbow):  r1 = L1*cos(theta2)
    #               x2 = r1*cos(theta1), y2 = r1*sin(theta1)
    #               z2 = L0 + L1*sin(theta2)
    #   p3 (EE):     r2 = L1*cos(theta2) + L2*cos(theta2+theta3)
    #               x3 = r2*cos(theta1), y3 = r2*sin(theta1)
    #               z3 = L0 + L1*sin(theta2) + L2*sin(theta2+theta3)
    #   반환: (x0,y0,z0), (x1,y1,z1), (x2,y2,z2), (x3,y3,z3)
    # ──────────────────────────────────────────────────────────
    raise NotImplementedError("TODO: forward_kinematics 를 구현하세요")
