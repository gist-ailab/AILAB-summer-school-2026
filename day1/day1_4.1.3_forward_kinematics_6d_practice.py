...
def forward_kinematics_ur5(thetas):
    ...
    """
    # ──────────────────────────────────────────────────────────
    # [실습] DH 변환을 곱해 나가며 UR5 정운동학을 구현하세요.
    #   (dh_matrix() 함수와 UR5_PARAMS 는 위에 제공돼 있음)
    #   개념: 6축 FK = 관절별 DH 변환행렬 A_i 의 누적 곱.
    #         T_i = A_1 @ A_2 @ ... @ A_i.
    #         각 T_i 의 위치는 T_i[:3, 3], 마지막 T_6 의 회전은 T_6[:3, :3] = EE 자세.
    #   1) T = 4x4 단위행렬(np.eye(4))로 시작
    #   2) frames = [T.copy()]   # frames[0] = base 프레임
    #   3) i = 0..5 반복:
    #        param = UR5_PARAMS[i]
    #        A = dh_matrix(thetas[i], param['d'], param['a'], param['alpha'])
    #        T = T @ A                     # 누적 곱
    #        frames 에 T.copy() 추가        # 그 관절까지의 누적 프레임
    #   4) joints_pos = [f[:3, 3] for f in frames]   # 각 프레임의 위치
    #      R_EE       = frames[-1][:3, :3]           # EE 자세
    #   5) 반환: joints_pos, R_EE, frames
    # ──────────────────────────────────────────────────────────
    raise NotImplementedError("TODO: forward_kinematics_ur5 를 구현하세요")
