...
def inverse_kinematics(x, y, z, L0=0.5, L1=1.0, L2=1.0):
    ...
    # ──────────────────────────────────────────────────────────
    # [실습] 3D 역운동학을 구현하세요.
    #   핵심: yaw(theta1)로 방향을 먼저 정하면, 나머지는 (r, z') 세로 평면의
    #         2링크 IK(= 4.2.1 과 동일한 코사인 법칙)로 바뀜.
    #   1) theta1 = atan2(y, x)   (x=y=0 이면 방향 미정 → 0)
    #   2) r = sqrt(x^2 + y^2),   z_prime = z - L0   (shoulder 기준 높이)
    #   3) cos_theta3 = (r^2 + z'^2 - L1^2 - L2^2) / (2*L1*L2)
    #        |cos_theta3| > 1 이면 도달 불가 → return None, None
    #   4) sin = ±sqrt(1 - cos^2) → 두 해(Elbow-down/Up),  theta3 = atan2(sin, cos)
    #   5) theta2 = atan2(z_prime, r) - atan2(L2*sin_theta3, L1 + L2*cos_theta3)
    #   반환: (theta1, theta2, theta3) 두 해 (theta1 은 공통)
    # ──────────────────────────────────────────────────────────
    raise NotImplementedError("TODO: inverse_kinematics 를 구현하세요")
