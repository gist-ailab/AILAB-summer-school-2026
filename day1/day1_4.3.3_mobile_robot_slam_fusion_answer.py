...
def pcd_from_rgbd_vectorized(rgb_arr, depth_arr, intrinsics, max_depth=3.5):
    ...
    """
    # 카메라 내부 파라미터(intrinsic): 초점거리(fx, fy)와 주점(cx, cy)
    fx = intrinsics[0, 0]
    fy = intrinsics[1, 1]
    cx = intrinsics[0, 2]
    cy = intrinsics[1, 2]
    ...
    # 유효한 depth 만 사용 (0=측정실패 제외, 너무 먼 값은 노이즈로 제외 / depth 단위 mm)
    ...
    # 핀홀 역투영: 픽셀(u,v)+깊이(z) → 카메라 좌표계 (x, y)
    x_c = (u_c - cx) * z_c / fx
    y_c = (v_c - cy) * z_c / fy

    # 좌표(N, 3)와 색상(N, 3, 0~1 정규화)을 묶어서 반환
