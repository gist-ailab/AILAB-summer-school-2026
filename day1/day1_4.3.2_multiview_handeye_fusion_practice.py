...
def make_pcd(rgb, depth_m, K, tf_data, T_ee_camlink):
    ...
    """
    # ──────────────────────────────────────────────────────────
    # [실습] 한 뷰의 RGB-D 를 world 좌표계 point cloud 로 변환하세요.
    #   좌표계 체인 (오른쪽부터 적용):
    #     p_world = base_to_ee @ T_ee_camlink @ OPTICAL_TO_CAMLINK @ p_optical
    #       - OPTICAL_TO_CAMLINK : 광학 좌표계 점 → camera_link (고정 변환, 제공됨)
    #       - T_ee_camlink       : hand-eye (camera_link 점 → franka_ee 점, 제공됨)
    #       - base_to_ee         : 로봇 FK (EE → base/world), 뷰마다 다름
    #   1) pts_optical, vs, us = backproject(depth_m, K)   # (N,4) 동차좌표
    #   2) base_to_ee = get_base_to_ee(tf_data)
    #   3) T_base_camlink = base_to_ee @ T_ee_camlink
    #      pts_world = (T_base_camlink @ OPTICAL_TO_CAMLINK @ pts_optical.T).T[:, :3]
    #   4) o3d PointCloud 생성:
    #        pcd.points = Vector3dVector(pts_world)
    #        색상: pcd.colors = Vector3dVector(rgb[vs, us]/255)
    #   반환: pcd
    # ──────────────────────────────────────────────────────────
    raise NotImplementedError("TODO: make_pcd 를 구현하세요")
