...
def inverse_kinematics(x, y, L1=1.0, L2=1.0):
    ...
    """
    # ──────────────────────────────────────────────────────────
    # [실습] 2링크 평면 역운동학을 구현하세요.
    #   1) 코사인 제2법칙으로 theta2 의 cos:
    #        cos_theta2 = (x^2 + y^2 - L1^2 - L2^2) / (2*L1*L2)
    #   2) |cos_theta2| > 1 이면 도달 불가 → return None, None
    #   3) sin 은 +/- 두 부호 → 두 해(Elbow-down / Elbow-up):
    #        sin = +sqrt(1 - cos^2),  -sqrt(1 - cos^2)
    #        theta2 = atan2(sin, cos_theta2)
    #   4) 각 해의 theta1:
    #        theta1 = atan2(y, x) - atan2(L2*sin_theta2, L1 + L2*cos_theta2)
    #   반환: (theta1_해1, theta2_해1), (theta1_해2, theta2_해2)
    # ──────────────────────────────────────────────────────────
