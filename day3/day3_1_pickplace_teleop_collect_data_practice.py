#!/usr/bin/env python3
# ============================================================
#  [문제 6] 텔레옵 액션 · 델타 적분 → 액션 조립
#  2교시 · Teleop  |  저장 위치: day3_1_2_pickplace_teleop_collect_data.py
#  ── 할 일: 아래 TODO(문제6) 주석의 ??? 2곳을 채우세요.
#     키보드 증분을 절대 목표로 누적하고, IK 팔 액션(위치+자세)을 조립한다.
...
def make_action(
    ...
    # TODO(문제6) IK 팔 액션 슬롯에는 "목표 위치(3D) + 목표 자세 쿼터니언(4D)" 을 이어붙인 7D 벡터가 들어간다.
    #   target_pos 와 target_quat 을 하나로 이어붙이고 (1, 7) 배치 차원을 맞춰라.  (torch.cat(...) + unsqueeze(0))
    actions[:, slices["arm_action"]] = ???
...
def main() -> None:
    ...
    try:
        ...
        current_pos, current_quat = controller_frame_pose(env)
        target_pos = current_pos[0].clone()
        target_yaw = torch.tensor([0.0], device=env.device)
        print(f"[CHECK] initial controller pos={target_pos.tolist()}")
        print(f"[CHECK] initial controller quat(wxyz)={current_quat[0].tolist()}")
        print(f"[CHECK] fixed target quat(wxyz)={down_quat.tolist()}")
        align_down(env, slices, total_dim, target_pos, down_quat, args_cli.align_steps)
            ...
            while simulation_app.is_running():
                if reset_requested or env_steps >= args_cli.max_steps:
                    target_pos = reset_episode(
                        env, teleop, limiter, slices, total_dim, down_quat
                    )
                    target_yaw = torch.tensor([0.0], device=env.device)
                    success_steps = 0
                    env_steps = 0
                ...
                # TODO(문제6) 키보드는 "증분(delta) 명령" command[:3] 을 준다. 그러나 IK 는 "절대 목표 위치" 를 받는다.
                #   현재 목표 위치에 이번 증분을 더해 새로운 절대 목표 위치를 만들라. (누적 적분)
                target_pos = ???
