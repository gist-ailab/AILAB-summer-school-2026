#!/usr/bin/env python3
"""Day3-3.4 ANSWER: IsaacLab Mimic DataGenerator rollout.
...
parser = argparse.ArgumentParser(description="Roll out a mimic source HDF5 with isaaclab_mimic DataGenerator.")
...
class TBarPickPlaceMimicEnv(ManagerBasedRLMimicEnv):
    ...
    # DataGenerator가 현재 robot TCP pose를 object-centric 계산에 쓰도록 제공합니다.
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while True:
                ...
                if prev_attempts != mimic_generation.num_attempts:
                    ...
                    target = env.cfg.datagen_config.generation_num_trials
                    if mimic_generation.num_success >= target:
                        break
...
# 4. 메인 함수
# ============================================================================
# isaaclab_mimic DataGenerator rollout 전체 파이프라인을 실행합니다.
def main() -> None:
    ...
    setup = setup_async_generation(env=env, num_envs=args_cli.num_envs, input_file=str(source_file), success_term=success_term, pause_subtask=False)
    env_loop_until_done(env, setup)
    print(f"[DONE] mimic generated dataset: {args_cli.output_file}")
