# ============================================================
#  day3_1_1_pickplace_statemachine_collect_data.py  ·  정답(Answer)
#  Pick&Place 자율 수집 — 상태머신으로 시연을 자동 생성해 '성공분만' HDF5 로 저장.
# ============================================================
...
def main():
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            # 성공한 환경은 카운트 증가, 실패한 환경은 0으로 리셋
            success_step_count = torch.where(
                success_flags,
                success_step_count + 1,
                torch.zeros_like(success_step_count),
            )
            ...
            if len(done_env_ids) > 0:
                ...
                env.recorder_manager.set_success_to_episodes(
                    ids_list,
                    torch.ones((len(ids_list), 1), dtype=torch.bool, device=env.device),
                )
