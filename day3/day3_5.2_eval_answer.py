...
# legacy(normalize=True) 로 수집·전처리한 체크포인트 재현용 유틸.
import day3_5_legacy_img_norm as legacy

IMG_STATS = {}   # legacy 모델일 때만 채워짐(카메라별 정규화 상수). 비어 있으면 raw.
...
def obs_to_robomimic(obs_policy: dict, device: torch.device, frame_stack: int = 2) -> dict:
    ...
    for key, val in obs_policy.items():
        if val.ndim == 1:
            ...
            if v.shape[-1] == 4:
                v = v[..., :3]
            # legacy(normalize=True) 로 학습한 모델이면 학습과 동일 인코딩으로 변환한다.
            if IMG_STATS:
                v = legacy.encode_legacy(v, key, IMG_STATS, v.dtype == torch.uint8)
            v = v.float()
...
def main():
    ...
    # legacy(normalize=True) 로 수집한 체크포인트면 정규화 상수를 자동 해결한다
    # (사이드카 → 학습 hdf5). raw(normalize=False) 데이터면 빈 dict → 변환 없음.
    IMG_STATS = legacy.resolve_img_stats(args_cli.checkpoint, os.path.join(_day3_dir, "datasets"))
    if IMG_STATS:
        print(f"[5.2] legacy 인코딩 감지 -> 정규화 상수 적용: {sorted(IMG_STATS)}")
