#!/usr/bin/env python3
"""Day3-3.6 ANSWER: Multi-subtask source and mimic generation.
...
parser = argparse.ArgumentParser(description="Multi-subtask source + mimic generation.")
...
def estimate_approach_step(demo, threshold: float = 0.07) -> int | None:
    dist = np.linalg.norm(target_eef_xyz(demo)[:, :2] - object_xyz(demo)[:, :2], axis=1)
...
def monotonic_transition_steps(length: int, raw_steps: list[int | None], min_gap: int = 2) -> list[int]:
    ...
    for i, raw in enumerate(raw_steps):
        ...
        step = int(np.clip(step, min_allowed, max_allowed))
        out.append(step)
        prev = step
    return out
...
# ============================================================================
# multi-subtask source 생성 후 mimic generation을 실행합니다.
def main():
